...
class Server:
    def __init__(self, root):
        ...
        self.file_locks = {}  # Per-file locks for synchronization
    ...
    def receive_file(self, conn, filename, username):
        try:
            ...
            if received == file_size:
                ...
                # Initialize the lock for the new file
                if unique_filename not in self.file_locks:
                    self.file_locks[unique_filename] = ReadWriteLock()
    ...
    def delete_file(self, conn, filename, username):
        ...
        if unique_filename in self.files and self.files[unique_filename] == username:
            file_lock = self.file_locks.get(unique_filename)
            if not file_lock:
                file_lock = ReadWriteLock()
                self.file_locks[unique_filename] = file_lock

            # Try to acquire write lock with a timeout
            acquired = file_lock.acquire_write(timeout=5)
            if not acquired:
                conn.send(b"Error: File is currently being accessed. Please try again later.\n")
                self.log_message(f"Delete attempt by {username} for file {filename} failed: File is in use.")
                return
            ...
            try:
                ...
                del self.file_locks[unique_filename]
        ...
        for filename in os.listdir(self.storage_dir):
            if os.path.isfile(os.path.join(self.storage_dir, filename)):
                ...
                if len(parts) == 2:
                    ...
                    # Initialize the lock for each existing file
                    if filename not in self.file_locks:
                        self.file_locks[filename] = ReadWriteLock()
